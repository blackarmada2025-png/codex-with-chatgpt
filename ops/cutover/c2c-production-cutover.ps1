...
function New-ProductionAdapter {
 ...
 $adapter=[pscustomobject]@{events=@();rollbackInvoked=$false;rollbackResult=$null;authSnapshot=$null;watchdogBackup=$null;candidatePid=$null;r3Runtime=$null;watchdogPid=$null;task=$null;Preflight=$null;CaptureAuthStore=$null;PauseWatchdog=$null;StopR3=$null;VerifyPortReleased=$null;StartCandidate=$null;Checkpoint=$null;HandoffWatchdog=$null;StartWatchdog=$null;VerifyWatchdogR3=$null;TestLocalHealth=$null;TestTunnel=$null;TestPublicHealth=$null;AuthStoreIntegrity=$null;StopCandidate=$null;RestoreR3=$null;RestoreWatchdog=$null}
 $adapter.Preflight={foreach($file in @($CandidateCli,$OldGatewayCli,$installer,$auth,$runtimeConfig,$runtimeScript)){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw "PRECHECK_REQUIRED_FILE_MISSING:$file"}};try{$config=Get-Content -LiteralPath $runtimeConfig -Raw|ConvertFrom-Json -ErrorAction Stop;$adapter.task=Get-ExactScheduledTask $ScheduledTaskName;$watchdog=Get-BoundWatchdogProcess $adapter.task $runtimeScript $runtimeConfig;$adapter.watchdogPid=[int]$watchdog.ProcessId}catch{throw "PRECHECK_WATCHDOG_BINDING_FAILED:$($_.Exception.Message)"};if([string]$config.workspace -ne $Workspace -or [string]$config.workspaceId -ne $WorkspaceId -or [int]$config.requiredPort -ne $Port -or [string]$config.gatewayCli -ne $OldGatewayCli -or [string]$config.c2cStateDir -ne $C2CStateDir){throw 'PRECHECK_WATCHDOG_CONFIG_IDENTITY_MISMATCH'};return $true}.GetNewClosure()
 ...
 $adapter.VerifyWatchdogR3={try{$process=Get-BoundWatchdogProcess $adapter.task $runtimeScript $runtimeConfig;$config=Get-Content -LiteralPath $runtimeConfig -Raw|ConvertFrom-Json;$runtime=Get-Content -LiteralPath $runtimeFile -Raw|ConvertFrom-Json;return ([int]$process.ProcessId -eq [int]$adapter.watchdogPid -and [string]$config.gatewayCli -eq $OldGatewayCli -and [int]$runtime.port -eq $Port -and [string]$runtime.workspaceId -eq $WorkspaceId -and -not(Test-PortReleased $Port))}catch{return $false}}.GetNewClosure()
