...
function Test-ScheduledTaskAction($Task,[string]$RuntimeScript,[string]$RuntimeConfig) {
 return ([string]$Task.Execute -match '(?i)(powershell|pwsh)(\.exe)?$' -and [string]$Task.Arguments -like "*$RuntimeScript*" -and [string]$Task.Arguments -like "*$RuntimeConfig*")
}
function Get-WatchdogProcess([string]$RuntimeScript,[string]$RuntimeConfig) {
 @((Get-CimInstance Win32_Process -ErrorAction Stop|Where-Object { $_.Name -match '^(?i:powershell|pwsh)\.exe$' -and [string]$_.CommandLine -like "*$RuntimeScript*" -and [string]$_.CommandLine -like "*$RuntimeConfig*" }))
}
...
function New-ProductionAdapter {
 $runtimeDir=$WatchdogRuntimeDirectory;$runtimeConfig=Join-Path $runtimeDir 'c2c-production-watchdog.config.json';$runtimeScript=Join-Path $runtimeDir 'orbnexa-vault-c2c-prod-watchdog.ps1';$installer=Join-Path $PSScriptRoot '..\..\scripts\install-production-watchdog.ps1';$formalAuth=Join-Path $C2CStateDir (Join-Path 'auth' "$WorkspaceId.json");$legacyAuth=Join-Path $LegacyC2CStateDir (Join-Path 'auth' "$WorkspaceId.json");$runtimeFile=Join-Path $C2CStateDir (Join-Path 'runtime' "$WorkspaceId.json");$legacyRuntimeFile=Join-Path $LegacyC2CStateDir (Join-Path 'runtime' "$WorkspaceId.json")
 $adapter=[pscustomobject]@{events=@();rollbackInvoked=$false;rollbackResult=$null;legacyAuthSnapshot=$null;formalAuthSnapshot=$null;watchdogBackup=$null;candidatePid=$null;r3Runtime=$null;watchdogPid=$null;task=$null;Preflight=$null;CaptureAuthStore=$null;PauseWatchdog=$null;StopR3=$null;VerifyPortReleased=$null;StartCandidate=$null;Checkpoint=$null;HandoffWatchdog=$null;StartWatchdog=$null;VerifyWatchdogR3=$null;TestLocalHealth=$null;TestTunnel=$null;TestPublicHealth=$null;AuthStoreIntegrity=$null;StopCandidate=$null;RestoreR3=$null;RestoreWatchdog=$null}
 $adapter.Preflight={foreach($file in @($CandidateCli,$OldGatewayCli,$installer,$formalAuth,$legacyAuth,$runtimeConfig,$runtimeScript,$legacyRuntimeFile)){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw "PRECHECK_REQUIRED_FILE_MISSING:$file"}};try{$config=Get-Content -LiteralPath $runtimeConfig -Raw|ConvertFrom-Json -ErrorAction Stop;$adapter.task=Get-ExactScheduledTask $ScheduledTaskName;$watchdog=Get-BoundWatchdogProcess $adapter.task $runtimeScript $runtimeConfig;$adapter.watchdogPid=[int]$watchdog.ProcessId}catch{throw "PRECHECK_WATCHDOG_BINDING_FAILED:$($_.Exception.Message)"};if([string]$config.workspace -ne $Workspace -or [string]$config.workspaceId -ne $WorkspaceId -or [int]$config.requiredPort -ne $Port -or [string]$config.gatewayCli -ne $OldGatewayCli -or (([string]$config.c2cStateDir) -and [string]$config.c2cStateDir -ne $LegacyC2CStateDir)){throw 'PRECHECK_LEGACY_WATCHDOG_IDENTITY_MISMATCH'};return $true}.GetNewClosure()
 ...
 $adapter.VerifyWatchdogR3={try{$process=Get-BoundWatchdogProcess $adapter.task $runtimeScript $runtimeConfig;$config=Get-Content -LiteralPath $runtimeConfig -Raw|ConvertFrom-Json;$runtime=Get-Content -LiteralPath $legacyRuntimeFile -Raw|ConvertFrom-Json;return ([int]$process.ProcessId -eq [int]$adapter.watchdogPid -and [string]$config.gatewayCli -eq $OldGatewayCli -and [string]$config.c2cStateDir -eq $LegacyC2CStateDir -and [int]$runtime.port -eq $Port -and [string]$runtime.workspaceId -eq $WorkspaceId -and -not(Test-PortReleased $Port))}catch{return $false}}.GetNewClosure()
 ...
 $adapter.RestoreWatchdog={if($adapter.watchdogBackup){Get-ChildItem -LiteralPath $adapter.watchdogBackup -File -ErrorAction Stop|Copy-Item -Destination $runtimeDir -Force -ErrorAction Stop}}.GetNewClosure()
